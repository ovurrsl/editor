...
export function resolveToolFrame(sceneApi: SceneApi, activeLevelId: AnyNodeId | null): ToolFrame {
  ...
  // The upstream drawing view is pinned to 'floor-plan'; the site-plan view only
  // exists on construction-documents/main, so this stays false here.
  const sitePlanView = (useDrawingView.getState().drawingType as string) === 'site-plan'
  return {
  ...
  for (const node of Object.values(sceneApi.nodes()) as unknown[]) {
    if (isUtilityPole(node)) {
      ...
      out.push({
        ...
        site: polePlan(node),
        overheadHeight:
          polePosition3(node)[1] +
          Math.max(0, (node.height || DEFAULT_POLE_HEIGHT) - POLE_CROSSARM_DROP),
        kind: 'pole',
      })
