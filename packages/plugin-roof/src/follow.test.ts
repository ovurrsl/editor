...
function house(): Record<string, Record<string, unknown>> {
  return {
    level_1: {
      id: 'level_1',
      type: 'level',
      children: ['wall_a', 'wall_b', 'roof_main', 'roof_porch'],
    },
    wall_a: {
      id: 'wall_a',
      type: 'wall',
    ...
    roof_main: {
      ...
      metadata: {
        ...
        autoRoof: {
          source: 'pascal:generate',
          intent: { form: 'gable', pitchInTwelfths: 8, overhangIn: 14, gables: ['left', 'right'] },
        },
      },
    },
    roof_porch: {
...
describe('which roof follows the walls', () => {
  ...
  test("the generator's intent reads back in the engine's terms; the command's options too", () => {
    ...
    const derived = {
      ...
      type: 'roof',
      metadata: {
        autoRoof: { options: { form: 'hip', pitchTwelfths: 5, overhangIn: 18, style: '' } },
      },
    }
