    ...
    <div className="flex flex-col text-sidebar-foreground">
      <div className="p-3">
        ...
        <p className="text-[11px] text-sidebar-foreground/60">
          Derives the roof from the level’s exterior walls: the wall loop becomes masses, each mass
          a segment seated on the plate, gable or hip by the style and the massing, a shed rising
          away from the street. Every exterior wall is told what it carries.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 px-3">
        <div>
          ...
          <select
            className={field}
            value={S.options.form}
            onChange={(e) => S.setOptions({ form: e.target.value as never })}
          >
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) => S.setOptions({ style: e.target.value })}
          >
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              S.setOptions({
                pitchTwelfths: Math.max(0.5, Math.min(18, Number(e.target.value) || 6)),
              })
            }
          />
        </div>
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              S.setOptions({ overhangIn: Math.max(0, Math.min(48, Number(e.target.value) || 0)) })
            }
          />
        </div>
      </div>
      ...
      {last && (
        <div className="mt-3 border-sidebar-border/60 border-t px-3 pt-3 text-[11px]">
          ...
          {last.errors.length > 0 ? (
            ...
            <p>
              {last.segments} segment{last.segments === 1 ? '' : 's'} over {last.masses} mass
              {last.masses === 1 ? '' : 'es'} · {Math.round(last.coverage * 100)}% of the footprint
              covered
