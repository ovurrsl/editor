...
export function traceExteriorLoop(walls: WallInput[]): Loop | null {
  ...
  for (let guard = 0; guard <= ext.length; guard++) {
    ...
    const curId = cur.id
    const next: { wall: WallInput; end: 0 | 1 } | undefined = (at.get(cornerKey(exit)) ?? []).find(
      (c) => c.wall.id !== curId,
    )
    if (!next) return null
...
export function decomposeRectilinear(polyF: readonly Pt[], minArea = 0.25): Bounds[] {
  ...
  for (let i = 0; i < na; i++) {
    ...
    for (let j = 0; j < nb; j++) {
      const c: Pt = [
        ((as[i] as number) + (as[i + 1] as number)) / 2,
        ((bs[j] as number) + (bs[j + 1] as number)) / 2,
      ]
  ...
  for (let round = 0; round < 32; round++) {
      ...
      for (let j0 = 0; j0 < nb; j0++) {
        ...
        for (let i1 = i0; i1 < na; i1++) {
          ...
          for (; j1 < nb; j1++) {
            ...
            if (!ok) break
            const area =
              ((as[i1 + 1] as number) - (as[i0] as number)) *
              ((bs[j1 + 1] as number) - (bs[j0] as number))
    ...
    for (let i = best.i0; i <= best.i1; i++)
      for (let j = best.j0; j <= best.j1; j++) (free[i] as number[])[j] = 0
    out.push({
...
export function coverageOf(polyF: readonly Pt[], rects: readonly Bounds[], step = 0.3): number {
  ...
  for (let a = bb.a0 + step / 2; a < bb.a1; a += step) {
    for (let b = bb.b0 + step / 2; b < bb.b1; b += step) {
      ...
      if (
        rects.some(
          (r) => a >= r.a0 - 0.01 && a <= r.a1 + 0.01 && b >= r.b0 - 0.01 && b <= r.b1 + 0.01,
        )
      )
        hit++
    }
