...
export async function getProjectModel(projectId: string): Promise<ActionResult<ProjectModelState>> {
  try {
    ...
    if (!modelToLoad && publishedVersion !== null) {
      ...
      modelToLoad =
        publishedModel && hasUsableSceneGraph(publishedModel.scene_graph) ? publishedModel : null
    }
...
export async function saveProjectModel(
  ...
  try {
    ...
    await (supabase.from('projects') as any).update({ is_empty: isEmpty }).eq('id', projectId)
...
async function createNextDraftVersion(
  ...
  const { data: newDraftModel, error: createError } = (await (
    supabase.from('projects_models') as any
  )
    .insert(insertData)
    .select()
  ...
  try {
    ...
    if (draftModel) {
      ...
      if (draftDiffersFromSaved) {
        const { data: lockedModel, error: lockDraftError } = (await (
          supabase.from('projects_models') as any
        )
          .update({
            draft: false,
    ...
    if (shouldPublish) {
      ...
      if (publishedVersion !== latestSavedModel.version) {
        const { error: updateProjectError } = await (supabase.from('projects') as any)
          .update({
