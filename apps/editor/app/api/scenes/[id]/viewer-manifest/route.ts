...
function isPointInPolygon(pt: [number, number], vs: [number, number][]): boolean {
  ...
  try {
    ...
    if (!stored) {
      ...
      if (staticFile) {
        try {
          ...
          stored = {
            id,
            name: raw.name || (id === '6c5728d1aed7' ? 'g�zeller Depo' : `Depo ${id}`),
            graph: {
              nodes: raw.nodes || {},
    ...
    const isGuzeller = resolvedSiteName.toLowerCase().includes('g�zeller') || resolvedSiteName.toLowerCase().includes('g�zeller') || id === '6c5728d1aed7';
